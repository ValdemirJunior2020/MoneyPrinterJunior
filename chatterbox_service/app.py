...
app = FastAPI(title="Chatterbox Emotion Service", version="1.0.0")
...
def _intensity(profile: EmotionProfile, style: str, index: int, total: int, segment: str) -> float:
    if total <= 1:
        ...
        factor = 1.0 + (phase - 0.35) * profile.intensity_variation
    if style == "dramatic":
        factor += 0.10 if segment.rstrip().endswith("!") else (0.06 if index == total - 1 else 0.0)
    elif style == "sermon":
        factor += 0.08 if index == total - 1 else (0.04 if segment.rstrip().endswith(("!", ":")) else 0.0)
...
def _synthesize_sync(text: str, style: str, reference_path: str | None = None) -> tuple[bytes, str]:
    ...
    for i, segment in enumerate(segments):
        kwargs = {
            "exaggeration": _intensity(profile, key, i, len(segments), segment),
        ...
        if i < len(segments) - 1:
            pause = _pause_seconds(key, segment, profile.pause_scale)
            audio_parts.append(np.zeros(max(1, round(model.sr * pause)), dtype=np.float32))
...
async def synthesize(request: SynthesizeRequest):
    try:
        audio, key = await asyncio.to_thread(_synthesize_sync, request.text, request.style, None)
    except Exception as exc:
        raise HTTPException(500, str(exc))
    return Response(
        content=audio,
        media_type="audio/wav",
        headers={"x-tts-engine": "chatterbox", "x-tts-profile": key},
    )
...
async def synthesize_upload(
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            ...
            temp_path = tmp.name
        audio, key = await asyncio.to_thread(_synthesize_sync, text, style, temp_path)
    except Exception as exc:
        raise HTTPException(500, str(exc))
    finally:
        if temp_path:
