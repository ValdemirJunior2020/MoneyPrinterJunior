...
def ken_burns_filter(
    ...
    return (
        ...
        f"zoompan=z='{z}':d={frames}:s={width}x{height}:fps=30,"
        "format=yuv420p"
    )
...
async def _concat_crossfade(
    ...
    for i in range(1, len(clips)):
        ...
        parts.append(
            f"{current}[{i}:v]"
            f"xfade=transition=fade:duration={overlap}:offset={offset:.3f}"
            f"{out_label}"
        )
...
def _safe_subtitle_size(
    requested_size: int | float,
    aspect: str,
) -> int:
    """
    Keep subtitle sizes sane for both horizontal and vertical video.

    Vertical 9:16 video is especially sensitive because libass can render
    SRT font sizes much larger than expected on a tall 1080x1920 frame.
    """
    try:
        size = int(round(float(requested_size)))
    except (TypeError, ValueError):
        size = 16 if aspect == "9:16" else 20

    if aspect == "9:16":
        return max(8, min(size, 34))

    return max(10, min(size, 42))
...
def _subtitle_margin_v(
    ...
    """
    Give Top/Bottom subtitles a safe distance from the frame edge.

    Middle subtitles do not need a large vertical margin because ASS
    alignment 5 places them in the center.
    """
    if position == "middle":
        return 0

    if aspect == "9:16":
        # Keep vertical subtitles away from TikTok/Reels UI areas.
        if font_size <= 12:
            return 95
        if font_size <= 16:
            return 115
...
def subtitle_force_style(
    ...
    font_size = _safe_subtitle_size(
        settings.size,
        aspect,
    )

    margin_v = _subtitle_margin_v(
        position,
        aspect,
        font_size,
    )
    ...
    border_style = 3 if settings.background else 1

    # Extra horizontal margin helps prevent words from touching the edge.
    margin_lr = 70 if aspect == "9:16" else 90

    return (
        f"FontName={settings.font},"
        f"FontSize={font_size},"
        f"PrimaryColour={_ass_color(settings.foreground_color)},"
        f"OutlineColour={_ass_color(settings.stroke_color)},"
        f"Outline={settings.stroke_width},"
        f"Shadow=0,"
...
async def render_video(
    ...
    if music_path and music_settings and music_settings.enabled:
        ...
        fade_out_start = max(
            0.0,
            final_duration - music_settings.fade_out,
        )

        filters.append(
            f"[2:a]"
            f"volume={music_settings.volume},"
            f"afade=t=in:st=0:d={music_settings.fade_in},"
            f"afade=t=out:st={fade_out_start:.3f}:d={music_settings.fade_out}"
            f"[music]"
        )
    ...
    if (
        ...
        escaped_subtitle_path = _escape_subtitle_path(
            subtitle_path
        )
